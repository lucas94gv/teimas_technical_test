...
RSpec.describe ApiFootballService do
  ...
  describe '.team_info' do
    it 'devuelve la información de un equipo por id' do
      stub_request(:get, /teams/).to_return(
        ...
        body: {
          "response": [
            { "team" => { "id" => 529, "name" => "Barcelona", "founded" => 1899 } }
          ]
        }.to_json
      )
  ...
  describe '.team_fixtures' do
    it 'devuelve fixtures de un equipo' do
      stub_request(:get, /fixtures/).to_return(
        ...
        body: {
          "response": [
            { "fixture" => { "id" => 100, "date" => "2026-03-01T18:00:00+00:00" } }
          ]
        }.to_json
      )

      result = ApiFootballService.team_fixtures(@team_id)
      expect(result.first["fixture"]["id"]).to eq(2001)
      expect(result.first["fixture"]["date"]).to eq("2024-03-01T20:00:00+01:00")
    end
  end
  ...
  #       body: {
  #         "response": [
  #           { "venue" => { "id" => @venue_id, "name" => "Camp Nou", "city" => "Barcelona" } }
  #         ]
  #       }.to_json
  #     )
  #
  #     result = ApiFootballService.venue_info(@venue_id)
  #     expect(result.first["name"]).to eq("Camp Nou")
  #     expect(result.first["city"]).to eq("Barcelona")
  #   end
  # end
end